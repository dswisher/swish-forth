...
blocks. From this point forward, all development happens in Neovim with
real Forth syntax.

## Background

With `BLOCK` and `LOAD` working, the remaining standard words can be
written in Forth and loaded at startup. The kernel's `_start` will
eventually just set up the stacks, load a bootstrap block, and enter the
interpreter.
