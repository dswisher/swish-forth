...
the block region in `.rodata`.  `blocks.s` is generated by `mkblocks.py`
and is not checked into git (it lives in `.gitignore`).  When no `.fth`
source files exist in `blocks/`, the script emits a single zero-filled
block so the kernel still builds.

### BLOCK source files (`blocks/block-NN.fth`)

Each file in the `blocks/` directory matching `block-NN.fth` (e.g.
`block-00.fth`, `block-01.fth`) corresponds to one Forth screen.  The
file must follow a strict 16×64 layout:

- At most 16 lines; fewer are padded with blank lines
- Each line at most 64 characters after stripping trailing whitespace
- Lines shorter than 64 characters are space-padded to exactly 64
- Non-ASCII characters are rejected
- Gaps in block numbering are filled with zero bytes
...
`THRU` depends on `DO`/`LOOP`/`I`, which are part of the core word
set implemented in step 12.  The definition above is included here
for documentation but is not yet compiled into the kernel — the assembler
would not resolve the missing CFA labels.
...
- `blocks/block-00.fth` — sample block with a colon definition (new)
...
- `blocks.s` — generated assembly containing block data (generated, not
  checked in; `make` runs `mkblocks.py` to produce it)
- `forth.s` — add `PREPARE-EVAL`, `RESTORE-SOURCE`, `BLOCK`,
  `EVALUATE`, `LOAD` (update)
- `forth.inc` — increase `IN_BUF_SIZE` from 256 to 1024 to accommodate
  one full block (update)
- `Makefile` — add `blocks.s` to the source list and a rule to run
  `mkblocks.py` (update)
